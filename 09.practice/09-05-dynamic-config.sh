...
# 이미지 빌드
docker build -t leafy-frontend:4.0.0-env . 
...
docker run -d -e BACKEND_HOST=leafy-dev -p 80:80 --name leafy-frontend --network leafy-network leafy-frontend:4.0.0-env

# 실제 적용된 nginx.conf 파일 내용 확인
docker exec leafy-frontend cat /etc/nginx/conf.d/default.conf
...
# 컨테이너 및 볼륨, 네트워크 삭제
docker rm -f leafy-postgres leafy-dev leafy-frontend
